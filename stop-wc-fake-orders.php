...
if ( ! defined( 'SWFO_SOFT_DENY_429' ) ) {
	define( 'SWFO_SOFT_DENY_429', true ); // soft deny (429) for CIDR/country lists
}

// Redis auth/db constants (preferred for secrets; options UI also supported)
if ( ! defined( 'SWFO_USE_REDIS' ) ) {
	define( 'SWFO_USE_REDIS', false ); // set true in wp-config for prod if phpredis is installed
}

if ( ! defined( 'SWFO_REDIS_AUTH' ) ) {
	define( 'SWFO_REDIS_AUTH', '' );  // e.g. 'mySecretPass' (leave '' if none)
}

if ( ! defined( 'SWFO_REDIS_DB' ) ) {
	define( 'SWFO_REDIS_DB', 0 );     // e.g. 0..15
}
...
class SWFO_Plugin {
	...
	private $defaults = array(
		'api_keys'                   => array(),                 // [id => password_hash(key)]
		'allowlist_cidrs'            => array(),          // [ '1.2.3.0/24', ... ]
		'ua_denylist'                => array(),              // substrings
		...
		'bypass_tokens'              => array(),            // ['token_name' => 'plain-or-hash']
		// CIDR soft-deny (429), separate from allowlist
		'soft_deny_cidrs'            => array(),          // returns 429 (not 403) to discourage probing
		// API hit logging
	...
	function init() {
		...
		add_action( 'load-woocommerce_page_swfo', array( $this, 'add_help_tabs' ) );

		// Core hooks
		add_action( 'admin_notices', array( $this, 'maybe_wc_notice' ) );
		add_action( 'admin_init', array( $this, 'ensure_defaults' ) );
		...
		add_action( 'admin_post_swfo_generate_key', array( $this, 'handle_generate_key' ) );
		add_action( 'admin_post_swfo_delete_key', array( $this, 'handle_delete_key' ) );
		add_action( 'admin_post_swfo_add_bypass', array( $this, 'handle_add_bypass' ) );
		add_action( 'admin_post_swfo_delete_bypass', array( $this, 'handle_delete_bypass' ) );

		// REST gating
		add_filter( 'rest_request_before_callbacks', array( $this, 'rest_gate' ), 10, 3 );
		...
		add_action( 'woocommerce_thankyou', array( $this, 'success_reset_by_order_id' ) );
		add_action( 'woocommerce_order_status_processing', array( $this, 'success_reset_by_order_obj' ) );
		add_action( 'woocommerce_order_status_completed', array( $this, 'success_reset_by_order_obj' ) );
		...
		add_filter( 'rest_request_before_callbacks', array( $this, 'rest_hit_logger' ), 1, 3 );

		// Admin actions for API hits (CSV export / clear)
		add_action( 'admin_post_swfo_export_hits', array( $this, 'handle_export_api_hits' ) );
		add_action( 'admin_post_swfo_clear_hits', array( $this, 'handle_clear_api_hits' ) );

		// Admin assets + AJAX endpoints
		add_action( 'admin_enqueue_scripts', array( $this, 'admin_enqueue_assets' ) );
		add_action( 'wp_ajax_swfo_get_events', array( $this, 'ajax_get_events' ) );
	...
	public function add_help_tabs() {
		...
		if ( ! $screen || $screen->id !== 'woocommerce_page_swfo' ) {
			return;
		}
	...
	function setup_redis() {
		...
		if ( ! $opt_use || ! extension_loaded( 'redis' ) ) {
			return;
		}
	...
	function admin_page() {
		...
		$keys   = get_option( SWFO_Opt::k( 'api_keys' ), array() );
		$bypass = get_option( SWFO_Opt::k( 'bypass_tokens' ), array() );
		$logs   = $this->logs_get();
	...
	function handle_generate_key() {
		...
		wp_redirect( admin_url( 'admin.php?page=swfo' ) );
		exit;
	}
	...
	function handle_delete_key() {
		if ( ! current_user_can( 'manage_options' ) || ! wp_verify_nonce( $_GET['_wpnonce'] ?? '', 'swfo_del_key' ) ) {
			wp_die( 'Forbidden' );
		}
	...
	function handle_add_bypass() {
		if ( ! current_user_can( 'manage_options' ) || ! check_admin_referer( 'swfo_add_bypass' ) ) {
			wp_die( 'Forbidden' );
		}
		...
		$bp          = get_option( SWFO_Opt::k( 'bypass_tokens' ), array() );
		$bp[ $name ] = $tok;
	...
	function handle_delete_bypass() {
		if ( ! current_user_can( 'manage_options' ) || ! wp_verify_nonce( $_GET['_wpnonce'] ?? '', 'swfo_del_bp' ) ) {
			wp_die( 'Forbidden' );
		}
	...
	private function is_same_origin( $request ) {
		...
		if ( ! $hdr ) {
			return false;  // block when no Origin/Referer is sent
		}
		$p = parse_url( $hdr );
	...
	public function rest_gate( $result, $handler, $request ) {
		...
		if ( $this->is_banned_ip( $ip ) ) {
			...
			return new WP_Error( 'swfo_banned', 'Forbidden', array( 'status' => 403 ) );
		}
		...
		if ( $is_store ) {
			...
			if ( $mode === 'same-origin' ) {
				if ( ! $this->is_same_origin( $request ) ) {
					...
					return new WP_Error( 'swfo_store_same_origin', 'Same-origin required', array( 'status' => 403 ) );
				}
			} elseif ( $mode === 'js-cookie' ) {
				$c = get_option( SWFO_Opt::k( 'required_cookie' ), 'swfo_js' );
				if ( empty( $_COOKIE[ $c ] ) ) {
					$this->log( 'storeapi_cookie_block', 'ip:' . $ip . ' route:' . $route );
					return new WP_Error( 'swfo_store_cookie', 'JS verification required', array( 'status' => 403 ) );
				}
			} elseif ( $mode === 'api-key' ) {
				$key = $request->get_header( 'x-wc-api-key' );
				if ( ! $key || ! $this->api_ok( $key ) ) {
					$this->log( 'storeapi_key_block', 'ip:' . $ip . ' route:' . $route );
					return new WP_Error( 'swfo_store_key', 'API key required', array( 'status' => 401 ) );
				}
				...
				if ( get_option( SWFO_Opt::k( 'enable_hmac' ), false ) ) {
					...
					if ( ! $this->hmac_ok( $request, $h, $ts ) ) {
						...
						return new WP_Error( 'swfo_bad_sig', 'Bad signature', array( 'status' => 403 ) );
					}
				}
			}
			...
			if ( $method === 'GET' ) {
				...
				if ( $rcnt > $store_limit ) {
					...
					return new WP_Error( 'swfo_store_rl', 'Too many requests', array( 'status' => 429 ) );
				}
			}
			...
			if ( $is_cart && $method !== 'GET' ) {
				...
				if ( $rc > $lim ) {
					...
					return new WP_Error( 'swfo_store_write_rl', 'Too many requests', array( 'status' => 429 ) );
				}
			}
		}
		/* -------------------------------------------------------------------- */
		...
		foreach ( (array) get_option( SWFO_Opt::k( 'allowlist_cidrs' ), array() ) as $cidr ) {
		...
		// Soft-deny CIDR (429).
		foreach ( (array) get_option( SWFO_Opt::k( 'soft_deny_cidrs' ), array() ) as $cidr ) {
			if ( $this->ip_in_cidr( $ip, $cidr ) ) {
				$this->log( 'soft_deny', 'ip:' . $ip . ' route:' . $route );
				return new WP_Error( 'swfo_soft_deny', 'Too many requests', array( 'status' => 429 ) );
			}
		}

		// UA denylist + minimal Origin/Referer sanity.
		$ua = $_SERVER['HTTP_USER_AGENT'] ?? '';
		foreach ( (array) get_option( SWFO_Opt::k( 'ua_denylist' ), array() ) as $deny ) {
			if ( stripos( $ua, $deny ) !== false ) {
				$this->touch_fail( 'ua', $ip );
				return new WP_Error( 'swfo_forbidden', 'Forbidden', array( 'status' => 403 ) );
			}
		}
		...
		if ( $key = $request->get_header( 'x-wc-api-key' ) ) {
			if ( ! $this->api_ok( $key ) ) {
				...
				return new WP_Error( 'swfo_bad_key', 'Invalid API key', array( 'status' => 401 ) );
			}
		}
		...
		if ( $count > (int) get_option( SWFO_Opt::k( 'ip_rate_limit' ), 50 ) ) {
			...
			return new WP_Error( 'swfo_rl', 'Too many requests', array( 'status' => 429 ) );
		}
		...
		if ( preg_match( '#/orders#', $route ) && $request->get_method() === 'POST' ) {
			...
			if ( get_option( SWFO_Opt::k( 'enable_honeypot' ), true ) ) {
				...
				if ( ! is_null( $hp ) && $hp !== '' ) {
					...
					return new WP_Error( 'swfo_hp', 'Forbidden', array( 'status' => 403 ) );
				}
			}
			...
			if ( get_option( SWFO_Opt::k( 'enable_js_challenge' ), true ) ) {
				...
				if ( empty( $_COOKIE[ $c ] ) ) {
					...
					return new WP_Error( 'swfo_js', 'JS verification required', array( 'status' => 403 ) );
				}
			}
			...
			if ( $email !== '' ) {
				...
				if ( $ecount > (int) get_option( SWFO_Opt::k( 'email_rate_limit' ), 10 ) ) {
					...
					return new WP_Error( 'swfo_rl_email', 'Too many attempts', array( 'status' => 429 ) );
				}
	 ...
	 */
	private function ban_key( $type, $val ) {
		return "ban:{$type}:" . md5( strtolower( trim( $val ) ) ); }
	...
	function checkout_validate() {
		...
		if ( get_option( SWFO_Opt::k( 'enable_honeypot' ), true ) && ! empty( $_POST['swfo_hp'] ) ) {
			wc_add_notice( 'Checkout validation failed.', 'error' );
			$this->log( 'hp_checkout', 'ip:' . $ip );
		}
		if ( get_option( SWFO_Opt::k( 'enable_js_challenge' ), true ) ) {
			$c = get_option( SWFO_Opt::k( 'required_cookie' ), 'swfo_js' ); if ( empty( $_COOKIE[ $c ] ) ) {
				wc_add_notice( 'Please enable JavaScript to continue.', 'error' );
				$this->log( 'cookie_checkout', 'ip:' . $ip );
			}
		}
	...
	private function api_ok( $key ) {
		foreach ( (array) get_option( SWFO_Opt::k( 'api_keys' ), array() ) as $id => $hash ) {
	...
	private function bypass_ok( $token ) {
		foreach ( (array) get_option( SWFO_Opt::k( 'bypass_tokens' ), array() ) as $n => $t ) {
	...
	private function hmac_ok( $req, $h, $ts ) {
		$sec = get_option( SWFO_Opt::k( 'hmac_secret' ), '' );
		if ( ! $sec || ! $h || ! $ts ) {
			return false;
		}
	...
	private function reset( $key ) {
		if ( $this->use_redis && $this->redis ) {
			try {
				$this->redis->del( 'swfo:' . $key );
				return;
			} catch ( \Throwable $e ) {
				error_log( 'SWFO Redis reset fallback: ' . $e->getMessage() );
				$this->use_redis = false; }
		}
	...
	private function logs_get() {
		if ( $this->use_redis && $this->redis ) {
			try {
				...
				foreach ( $raw as $r ) {
					$dec = unserialize( $r );
					if ( is_array( $dec ) && isset( $dec['t'], $dec['type'] ) ) {
						$o[] = $dec;
					}
				}
				return $o;
		...
		if ( $this->use_redis && $this->redis ) {
			try {
				...
				foreach ( array_reverse( $logs ) as $l ) {
					$this->redis->lPush( 'swfo:logs', serialize( $l ) ); }
				return;
			} catch ( \Throwable $e ) {
	...
	private function log( $type, $note = '' ) {
		...
		$this->logs_set( $l );
		if ( get_option( SWFO_Opt::k( 'log_to_error_log' ), false ) ) {
			error_log( "[SWFO] $type - $note" );
		}
		if ( $u = get_option( SWFO_Opt::k( 'webhook_url' ), '' ) ) {
			$this->post_webhook(
				$u,
				array(
					't'    => time(),
	 ...
	 * @return void
	 */
	private function post_webhook( $url, $payload ) {
		wp_remote_post(
			$url,
			array(
				'timeout'  => 2,
	...
	private function client_ip() {
		if ( ! empty( $_SERVER['HTTP_CF_CONNECTING_IP'] ) ) {
			return $_SERVER['HTTP_CF_CONNECTING_IP']; // CF
		}
	...
	private function ip_in_cidr( $ip, $cidr ) {
		...
		list($sub, $mask) = explode( '/', $cidr );
		$mask             = (int) $mask;
		$ipl              = ip2long( $ip );
		$s                = ip2long( $sub );
		if ( $ipl === false || $s === false ) {
			return false;
		}
		$m = -1 << ( 32 - $mask );
		return ( ( $ipl & $m ) === ( $s & $m ) );
	...
	function links( $links ) {
		array_unshift( $links, '<a href="' . admin_url( 'admin.php?page=swfo' ) . '">Settings</a>' );
		return $links; }
	...
	public function rest_hit_logger( $result, $handler, $request ) {
		...
		if ( ! get_option( SWFO_Opt::k( 'enable_api_hit_logging' ), true ) ) {
			return $result;
		}
		if ( ! ( $request instanceof WP_REST_Request ) ) {
			return $result;
	...
	private function api_hits_clear() {
		if ( $this->use_redis && $this->redis ) {
			try {
				$this->redis->del( 'swfo:api_hits' );
				return;
			} catch ( \Throwable $e ) {
				error_log( 'SWFO Redis hits_clear fallback: ' . $e->getMessage() );
				$this->use_redis = false; }
		}
	...
	public function handle_export_api_hits() {
		...
		// Header
		fputcsv( $out, array( 'datetime', 'ip', 'method', 'path', 'route', 'user_agent', 'data' ) );
		foreach ( $hits as $h ) {
			fputcsv(
				$out,
				array(
	...
	private function mask_sensitive( $val ) {
		$keys = array( 'password', 'pass', 'pwd', 'secret', 'token', 'api_key', 'apikey', 'key', 'authorization', 'auth', 'hmac', 'signature', 'x-wc-api-key', 'x-wc-hmac' );
		if ( is_array( $val ) ) {
			$out = array();
			foreach ( $val as $k => $v ) {
	...
	private function truncate_json( $data, $limit ) {
		...
		return json_decode( substr( $json, 0, $limit ), true ) ?: array( '_truncated_' => true );
	}
	...
	private function paginate_array( array $items, int $page, int $per_page ) {
		...
		return array( array_slice( $items, $offs, $per_page ), $page, $pages, $total );
	}
	 ...
	 */
	private function tab_url( $tab, array $args = array() ) {
	 ...
	 */
	private function render_pager( $tab, $arg_page, $page, $pages, array $extra = array() ) {
		if ( $pages <= 1 ) {
			return;
		}
		$q = array_merge( array( 'swfo_tab' => $tab ), $extra );
		echo '<div class="tablenav"><div class="tablenav-pages">';
		// First / Prev
		if ( $page > 1 ) {
			echo '<a class="button" href="' . esc_url( $this->tab_url( $tab, array_merge( $q, array( $arg_page => 1 ) ) ) ) . '">&laquo; First</a> ';
			echo '<a class="button" href="' . esc_url( $this->tab_url( $tab, array_merge( $q, array( $arg_page => ( $page - 1 ) ) ) ) ) . '">&lsaquo; Prev</a> ';
		} else {
		...
		// Next / Last
		if ( $page < $pages ) {
			echo '<a class="button" href="' . esc_url( $this->tab_url( $tab, array_merge( $q, array( $arg_page => ( $page + 1 ) ) ) ) ) . '">Next &rsaquo;</a> ';
			echo '<a class="button" href="' . esc_url( $this->tab_url( $tab, array_merge( $q, array( $arg_page => $pages ) ) ) ) . '">Last &raquo;</a>';
		} else {
 ...
 */
add_action( 'plugins_loaded', array( 'SWFO_Plugin', 'boot' ) );
